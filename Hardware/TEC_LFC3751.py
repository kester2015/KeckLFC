...
class TEC_LFC3751(Device):
    ...
    def Reset_Default(self):
        warnings.warn(self.devicename + ": To improve code stability, thermistor ABC parameter are not setted in Reset_Default function."+
    ...
    def Tset(self, Tset):
        ...
        print(self.devicename + ": " + "Temperature setted to "+ f"{Tset} degC.")
...
#         self._write_cmd_code(cmd_code=25, data=abc[2])
    ...
    def output(self, status):
        ...
        print(self.devicename + ": " + "setted Output status as " + status_dict[status] + ".")
...
#         self._write_cmd_code(cmd_code=31, data=Ilim_pos)
    ...
    def _query_cmd_code(self, cmd_code, data=0, read_data=True):
        ...
        if not int(response['command_code'])==int(cmd_code):
            ...
            if not count<self.__query_trial_max:
                warnings.warn(self.devicename + ": Query failed to get corresponding response in "+str(self.__query_trial_max)+" times."
                              +" Waiting to get response of cmd code "+str(cmd_code)+", the last response code is " + 
                              str(response['command_code']) +". Try add time.sleep(1) between commands." )
        return response
    ...
    def _generate_cmd_packet(self, cmd_code, data=0, read_data=False):
        if cmd_code == 54:
            raise ValueError(self.devicename+": cmd 54 is password set/read function, which is disabled in this code.")
    ...
    def _decode_response_packet(self, response):
        if not response[0]=='@':
            raise ValueError(self.devicename + ": Decode response error. response = " + str(response) + " should start with '@'.")
        ...
        result['RS232_address']=response[2:4] # RS-232 address of the unit as set on the front panel of the unit (00 is reserved)
