...
class PritelAmp(Device):
    ...
    def connect(self):
        if not self.connected:
            try:
                ...
                #     return 1
                # else:
                #     print(self.devicename + ": Connection Not ready. response: '" + response +"'.")
                #     return -1
            except:
                import sys
                e = sys.exc_info()[0]
    ...
    def preAmp(self, mA): 
        ...
        # print(self.devicename + ": " + response)
        if self.ramp_pre_ma == 0:
            response = self._setPreAmpStr(amp_mA = mA)
            print(self.devicename + ": " + response)
        else:
            print(self.devicename + ": " + f"Disable Ramping by self.ramp_pre_ma = 0, now {self.ramp_pre_ma} mA.")
    ...
    def pwrAmp(self, mA): 
        ...
        if self.ramp_pwr_ma == 0:
            ...
            print(self.devicename + ": " + f"Disable Ramping by self.ramp_pwr_ma = 0, now {self.ramp_pwr_ma} mA.")
    ...
    def activation(self, status):
        ...
        if not status.casefold() in ['0', '1', 'on', 'off']:
            raise ValueError(self.devicename+": "+"Activation status "+status+" not recognized. Should choose from 0|1|'ON'|'OFF'. ")
        if status == '0':
            status = 'OFF'
        elif status == '1':
            status = 'ON'
        if status == 'ON':  # Give a SAFETY NOTIFY every time before activating
            print(self.devicename + ": " + "ACTIVATING LASER PUMP (OUTPUT), MAKE SURE SEED INPUT POWER IS APPROPRIATE TO AVOID DAMAGE.")
        cmd = "FA "+status
        ...
        while not self.activation.casefold() == status.casefold():
            ...
            print(self.devicename + ": " + "......waiting Activation status set to " + status + ", now " + self.activation)
            self.query(cmd)
        print(self.devicename + ": " + "setted Activation status as " + self.activation + f", finished in {time.time()-timer_start:.3f} seconds")
    ...
    def _setPreAmpStr(self, amp_mA): # return 'Setting PreAmp Current to 030 mA'
        ...
        if amp_mA>self.__preAmpMax: # if >600mA
            raise ValueError(self.devicename+": preAmp set value "+f"{amp_mA:.0f}".zfill(3)+f" mA higher than max __preAmpMax = {self.__preAmpMax:.0f} mA.")
    ...
    def _setPwrAmpStr(self, amp_mA): # return 'Setting PowerAmp Current to 5.80 A'
        ...
        if not np.mod(amp_mA, 10)==0:
            warnings.warn(self.devicename+": preAmp set value "+f"{amp_mA:.0f}".zfill(3)+f" mA has higher precision than 10mA, will be rounded to "+f"{np.round(amp_mA/10)*10:.0f} mA.")
            amp_mA = np.round(amp_mA/10)*10
        if amp_mA>self.__pwrAmpMax: # if >5800mA
            raise ValueError(self.devicename+": preAmp set value "+f"{amp_mA:.0f}".zfill(3)+f" mA higher than max __pwrAmpMax = {self.__pwrAmpMax:.0f} mA.")
